$(document).ready(function() {
    $('#contactForm').submit(function(event) {
		if ($("input[name='nameTB']").val().length === 0 || !$.trim($("input[name='nameTB']").val()) ) {
			...
			$('#tgMsgFrame').prop('src', 'https://api.telegram.org/bot' + $botToken + '/sendMessage?chat_id=-365041799&text=Enquiry from ' + $("input[name='nameTB']").val() + '%0D%0AEmail: ' + $("input[name='emailTB']").val() + '%0D%0AEnquiry Type: ' + $("input[name='enquiryType']").val() + '%0D%0A%0D%0AMessage:%0D%0A' + $("#messageTB").val().replace(/\n\r?/g, '%0D%0A') );
